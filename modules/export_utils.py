...
# ==========================================
# 1. EXPORTADORES DO PLANEJAMENTO TÁTICO
# ==========================================
def formatar_planilha_tatica(writer, sheet_name, df_orig):
    """Pinta o Cabeçalho NIP, Fundo Laranja (Super Ponto) e Letra Vermelha (Prioridade)."""
    ...
    fill_superponto = PatternFill(start_color='FCE4D6', end_color='FCE4D6', fill_type='solid') # Laranja Claro
    font_prioridade = Font(color='FF0000', bold=True, name='Calibri') # Vermelho
    ...
    for r_idx, row in enumerate(worksheet.iter_rows(min_row=2), start=2):
        is_prio = col_prio_idx and str(row[col_prio_idx - 1].value) == 'Sim'
        is_sp = col_sp_idx and str(row[col_sp_idx - 1].value).startswith('SIM')
...
def gerar_excel_tatica(df, colunas_originais=None):
    output = io.BytesIO()
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        formatar_planilha_tatica(writer, 'Obras Roteirizadas', df_saida)
    return output.getvalue()

def limpar_colunas_tatica(df_alvo, cols_originais):
...
def gerar_kml_tatica(df_kml, nome_arquivo, colunas_exibir, bases_ativas, funcao_formatadora):
    ...
    for b in bases_ativas:
        ...
        for p in df_b['PERIODO'].unique():
            ...
            for _, r in df_p.iterrows():
                ...
                cor = "s_red" if str(r.get('PRIORIDADE')) == 'Sim' else "s_blue"
...
# ==========================================
# 2. EXPORTADORES DA FISCALIZAÇÃO
# ==========================================
